...
function detect(){
  try{
    const l=((navigator.languages&&navigator.languages[0])||navigator.language||"en-US").toLowerCase();
...
async function fetchRates(base,targets){
  try{
    ...
    const c=sget(k); if(c)return c;

    const r=await fetch(`/api/rates?base=${base}&symbols=${targets.join(",")}`);
    if(!r.ok)return null;
...
async function loadSettings(){
  try{
    ...
    const r=await fetch(`/apps/currency-switcher/api/merchant-settings?shop=${encodeURIComponent(shop)}`);
    if(!r.ok)return null;
    return await r.json();
  }catch(e){return null;}
}
...
function applyPlacement(w,st){
  const fc=st?.fixedCorner||"top-right";
  const t=st?.distanceTop??16,r=st?.distanceRight??16,b=st?.distanceBottom??16,l=st?.distanceLeft??16;
...
async function init(){
  ...
  await runFor(i,st); // ✅ FORCE initial conversion
}
